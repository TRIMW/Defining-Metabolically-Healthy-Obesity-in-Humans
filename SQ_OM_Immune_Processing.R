...
OM_Immune <- subset(x = OM_merge, idents = c("T cells", "NK cells", "Myeloid cells", "B cells", "Plasma Cells"))
...
##Removing debris cluster in SQ Myeloid sample
SQ_ID <- c("LAMs", "cDC2B", "Monocytes", "PVMs", "cDC1", "cDC2A")
names(SQ_ID) <- levels(SQ_Myeloid)
SQ_Myeloid <- RenameIdents(SQ_Myeloid, SQ_ID)

##Renaming OM Myeloid
OM_ID <- c("THBS1+", "LAMs", "cDC2B", "Monocytes", "PVMs", "FBLN1+", "cDC2A")
names(OM_ID) <- levels(OM_Myeloid)
OM_Myeloid <- RenameIdents(OM_Myeloid, OM_ID)